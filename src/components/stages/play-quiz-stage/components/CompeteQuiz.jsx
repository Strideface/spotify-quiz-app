export default function CompeteQuiz() {
  return <h1>Compete Quiz</h1>
  // Render QuizTracks component here after setting up a prior component, which renders after selecting the game tile,
  // allowing the user to select a playlist (these will be pre-made playlists relating to different genres) and thus
  // QuizTracks will get the appropriate playlistHref
}
